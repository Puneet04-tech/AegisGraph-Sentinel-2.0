...
class EntityResolver:
    ...
    def _get_or_create_entity(
        self, entity_id: Optional[str], entity_type: EntityType, value: str, attributes: Optional[Dict[str, Any]] = None
    ) -> Tuple[Entity, bool]:
    ...
    def _calculate_confidence(
        self, relationship_type: RelationshipType, evidence: List[str], attributes: Dict[str, Any]
    ) -> float:
    ...
    def link_entities(self, request: LinkRequest) -> LinkResult:
        ...
        logger.info(
            f"Linked entity {source_entity.id} to {target_entity.id} with relationship {request.relationship_type.value}"
        )
    ...
    def link_wallet(
        self, account_id: str, wallet_address: str, is_owner: bool = True, confidence: float = 0.90
    ) -> LinkResult:
    ...
    def link_transaction(
        self, from_account: str, to_account: str, transaction_id: str, amount: float, confidence: float = 0.80
    ) -> Tuple[LinkResult, LinkResult]:
    ...
    def find_similar_entities(self, entity_id: str, threshold: float = 0.7) -> List[Entity]:
        ...
        for rel in connections:
            ...
            for second_rel in second_degree:
                ...
                if (
                    other_id != entity_id
                    and other_id not in {r.source_id for r in connections}
                    and other_id not in {r.target_id for r in connections}
                ):
    ...
    def unlink_entities(
        self, source_id: str, target_id: str, relationship_type: Optional[RelationshipType] = None
    ) -> bool:
