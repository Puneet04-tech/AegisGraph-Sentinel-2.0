...
class KnowledgeGraph:
    ...
    def remove_edge(self, source_id: str, target_id: str, relationship_type: Optional[RelationshipType] = None) -> bool:
        ...
        # Delete from store
        rel_id = (
            f"{source_id}:{target_id}:{relationship_type.value if relationship_type else rel.relationship_type.value}"
        )
        with self._store._relationship_lock:
    ...
    def traverse_bfs(
        self, start_id: str, max_depth: int = 3, relationship_type: Optional[RelationshipType] = None
    ) -> GraphQueryResult:
    ...
    def _traverse(
        self, start_id: str, order: TraversalOrder, max_depth: int, relationship_type: Optional[RelationshipType]
    ) -> GraphQueryResult:
